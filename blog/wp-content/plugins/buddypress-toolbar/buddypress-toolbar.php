<?php 
/**
 * Main plugin file. This plugin adds useful admin links and resources for BuddyPress to the WordPress Toolbar / Admin Bar.
 *
...
function ddw_bptb_admin_bar_menu() {
	...
	if ( ! is_user_logged_in() || 
		...
		defined( 'BPTB_DISPLAY' )  // allows for custom disabling
	)
		return;
	...
	if ( !defined( 'BPTB_RESOURCES_DISPLAY' ) ) {
		$bpgroup_menu_items = array(
			...
			'bpsupportwporg' => array(
				...
				'href'   => 'http://wordpress.org/tags/buddypress?forum_id=10',
	...
	if ( !defined( 'BPTB_DE_DISPLAY' ) && ( get_locale() == 'de_DE' || get_locale() == 'de_AT' || get_locale() == 'de_CH' || get_locale() == 'de_LU' ) ) {
		/** German BP forum */
	...
	if ( ! defined( 'BPTB_TRANSLATIONS_DISPLAY' ) && ( empty( $locale ) || !( get_locale() == 'en_US' || get_locale() == 'en_GB' || get_locale() == 'en_NZ' || get_locale() == 'en' ) ) ) {
		/** Translations Forum */
	...
	if ( ( function_exists( 'is_plugin_active' ) && is_plugin_active( 'buddypress/bp-loader.php' ) ) || class_exists( 'BP_Core' ) ) {
		 ...
		 * @since 1.0
		 */
		if ( !defined( 'BPTB_EXTENSIONS_DISPLAY' ) && current_user_can( 'activate_plugins' ) ) {
	 ...
	 */
		...
		$wp_admin_bar->add_menu( array(
			...
			'href'  => $bp_aurl_main,
			'meta'  => array( 'class' => esc_attr__( $bptb_main_item_icon_display ), 'title' => esc_attr__( $bptb_main_item_title_tooltip ) )
		) );
	...
	if ( ! defined( 'BPTB_MANAGE_GROUPS_DISPLAY' ) && ( ( function_exists( 'bp_is_active' ) && bp_is_active( 'groups' ) ) &&  $bptb_is_bpmg == 'bpmg_yes' ) ) {
	...
	$wp_admin_bar->add_group( array(
		...
		if ( !defined( 'BPTB_THEME_DISPLAY' ) && ( current_user_can( 'manage_options' ) || current_user_can( 'edit_theme_options' ) || current_user_can( 'switch_themes' ) ) ) {
			$wp_admin_bar->add_group( array(
				'parent' => $extgroup,
				'id'     => $tgroup,
			) );
...
dirname( __FILE__ ) ) );
	...
	<style type="text/css">
		...
		#wpadminbar .ab-top-menu > li.menupop.icon-buddypress > .ab-item {
      			background-image: url(<?php echo $bptb_main_icon; ?>);

...
define( 'BP_DASHBOARD_IS_INSTALLED', 1 );

// Define a constant that will hold the current version number of the component
// This can be useful if you need to run update scripts or do compatibility checks in the future
define( 'BP_DASHBOARD_VERSION', '1.0' );

// Define a constant that we can use to construct file paths throughout the component
define( 'BP_DASHBOARD_PLUGIN_DIR', dirname( __FILE__ ) );
 ...
 */
define ( 'BP_DASHBOARD_DB_VERSION', '1' );

/* Only load the component if BuddyPress is loaded and initialized. */
function bp_dashboard_init() {
	// Because our loader file uses BP_Component, it requires BP 1.5 or greater.
	if ( version_compare( BP_VERSION, '1.3', '>' ) )
		require( dirname( __FILE__ ) . '/includes/bp-dashboard-loader.php' );
}
add_action( 'bp_include', 'bp_dashboard_init' );

/* Put setup procedures to be run when the plugin is activated in the following function */
function bp_dashboard_activate() {

}
register_activation_hook( __FILE__, 'bp_dashboard_activate' );

/* On deacativation, clean up anything your component has added. */
function bp_dashboard_deactivate() {
	/* You might want to delete any options or tables that your component created. */
}
register_deactivation_hook( __FILE__, 'bp_dashboard_deactivate' );
?>

...
/**
 * bp_dashboard_load_template_filter()
 *
 ...
 *
 * This will become clearer in the function bp_dashboard_home() when you want to load
 * a template file.
 */
function bp_dashboard_load_template_filter( $found_template, $templates ) {
	global $bp;

	/**
	 * Only filter the template location when we're on the dashboard component pages.
	 */
	...
	return apply_filters( 'bp_dashboard_load_template_filter', $found_template );
}
add_filter( 'bp_located_template', 'bp_dashboard_load_template_filter', 10, 2 );
 ...
 * 	or functions in your bp-dashboard-classes.php file to fetch what you need. Spraying database
...
function bp_dashboard_accept_terms() {
	 ...
	 */
	check_admin_referer( 'bp_dashboard_accept_terms' );
	...
	bp_dashboard_record_activity( array(
		'type' => 'accepted_terms',
		'action' => apply_filters( 'bp_dashboard_accepted_terms_activity_action', sprintf( __( '%s accepted the really exciting terms and conditions!', 'bp-dashboard' ), $user_link ), $user_link ),
	) );

	/* See bp_dashboard_reject_terms() for an explanation of deleting activity items */
	...
	do_action( 'bp_dashboard_accept_terms', $bp->loggedin_user->id );
...
/**
 * bp_dashboard_reject_terms()
 *
 * Rejects the terms and conditions screen for the logged in user.
 * Records an activity stream item for the user.
 */
function bp_dashboard_reject_terms() {
	global $bp;

	check_admin_referer( 'bp_dashboard_reject_terms' );
	...
	bp_dashboard_record_activity( array(
		'type' => 'rejected_terms',
		'action' => apply_filters( 'bp_dashboard_rejected_terms_activity_action', sprintf( __( '%s rejected the really exciting terms and conditions.', 'bp-dashboard' ), $user_link ), $user_link ),
	) );
	...
	do_action( 'bp_dashboard_reject_terms', $bp->loggedin_user->id );

	return true;
}

/**
 * bp_dashboard_send_posts()
 *
 ...
 */
function bp_dashboard_send_posts( $to_user_id, $from_user_id ) {
	global $bp;

	check_admin_referer( 'bp_dashboard_send_posts' );
	 ...
	 * bp-dashboard-classes.php that would run the SQL query.
	 */
	delete_user_meta( $to_user_id, 'dashboard-posts' );
	/* Get existing fives */
	$existing_posts = maybe_unserialize( get_user_meta( $to_user_id, 'dashboard-posts', true ) );
	 ...
	 */
	if ( !in_array( $from_user_id, (array)$dashboard_posts ) ) {
		$existing_fives[] = (int)$from_user_id;

		/* Now wrap it up and fire it back to the database overlords. */
		update_user_meta( $to_user_id, 'dashboard-posts', serialize( $existing_posts ) );

		// Let's also record it in our custom database tables
		$db_args = array(
			'recipient_id'  => (int)$to_user_id,
			'poster_id' => (int)$from_user_id
		);

		$high_five = new BP_Dashboard_Posts( $db_args );
		$high_five->save();
	}
	 ...
	 * this notification in bp_dashboard_format_notifications() using the 'new_post' action.
	 */
	bp_core_add_notification( $from_user_id, $to_user_id, $bp->dashboard->slug, 'new_dashboard_post' );
	...
	bp_dashboard_record_activity( array(
		'type' => 'rejected_terms',
		'action' => apply_filters( 'bp_dashboard_new_post_activity_action', sprintf( __( '%s dashboard-posts %s!', 'bp-dashboard' ), $from_user_link, $to_user_link ), $from_user_link, $to_user_link ),
		'item_id' => $to_user_id,
	) );

	/* We'll use this do_action call to send the email notification. See bp-dashboard-notifications.php */
	do_action( 'bp_dashboard_send_post', $to_user_id, $from_user_id );

	return true;
}

/**
 * bp_dashboard_get_posts_for_user()
 *
 * Returns an array of user ID's for users who have high fived the user passed to the function.
 */
function bp_dashboard_get_posts_for_user( $user_id ) {
	global $bp;

	if ( !$user_id )
		return false;

	return maybe_unserialize( get_user_meta( $user_id, 'dashboard-posts', true ) );
}


/**
 * bp_dashboard_remove_data()
 *
 * It's always wise to clean up after a user is deleted. This stops the database from filling up with
 * redundant information.
 */
function bp_dashboard_remove_data( $user_id ) {
	...
	delete_user_meta( $user_id, 'bp_dashboard_some_setting' );

	do_action( 'bp_dashboard_remove_data', $user_id );
}
add_action( 'wpmu_delete_user', 'bp_dashboard_remove_data', 1 );
add_action( 'delete_user', 'bp_dashboard_remove_data', 1 );

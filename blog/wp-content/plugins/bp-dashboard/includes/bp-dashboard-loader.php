 ...
 * @since 1.0
 */
class BP_Dashboard_Component extends BP_Component {
	 ...
	 *		 to see how BP_DASHBOARD_PLUGIN_DIR was defined.
	 *
	 * @package BuddyPress_Dashboard_Component
	 * @since 1.0
	 */
	function __construct() {
		...
		parent::start(
			'dashboard',
			__( 'Dashboard', 'bp-dashboard' ),
			BP_DASHBOARD_PLUGIN_DIR
		);
	 ...
	 *       setup):
	 *           /wp-content/plugins/bp-dashboard/notifications.php
	 *           /wp-content/plugins/bp-dashboard/filters.php
	 ...
	 *           /wp-content/plugins/bp-dashboard/bp-dashboard/notifications.php
	 *           /wp-content/plugins/bp-dashboard/bp-dashboard/filters.php
	 ...
	 *           /wp-content/plugins/bp-dashboard/bp-dashboard/bp-dashboard-notifications.php
	 *           /wp-content/plugins/bp-dashboard/bp-dashboard/bp-dashboard-filters.php
	 *
	...
	function includes() {
		...
		// Files to include
		$includes = array(
			'includes/bp-dashboard-template.php',
			'includes/bp-dashboard-screens.php',
			'includes/bp-dashboard-functions.php',
			'includes/bp-dashboard-notifications',
			'includes/bp-dashboard-activity.php',
			'includes/bp-dashboard-classes.php',
			'includes/bp-dashboard-cssjs.php',
			'includes/bp-dashboard-ajax.php'
		);
		...
		if ( is_admin() || is_network_admin() ) {
			include( BP_DASHBOARD_PLUGIN_DIR . '/includes/bp-dashboard-admin.php' );
		}*/
	}
	 ...
	 *				  in the URL http://testbp.com/members/boone/dashboard, the
	 *				  'dashboard' portion of the URL is formed by the 'slug'.
	 *				  Site admins can customize this value by defining
	 *				  BP_DASHBOARD_SLUG in their wp-config.php or bp-custom.php
	 *				  files.
	 ...
	 *				    http://testbp.com/dashboard/test
	 *				  'dashboard' is a root slug. This should always be defined
	 ...
	 *				  directory, such as http://testbp.com/dashboard. When
	...
	function setup_globals() {
		...
		if ( !defined( 'BP_DASHBOARD_SLUG' ) )
			define( 'BP_DASHBOARD_SLUG', $this->id );
		...
		$global_tables = array(
			'table_name'      => $bp->table_prefix . 'bp_dashboard'
		);
		
		// Set up the $globals array to be passed along to parent::setup_globals()
		$globals = array(
			'slug'                  => BP_DASHBOARD_SLUG,
			'root_slug'             => isset( $bp->pages->{$this->id}->slug ) ? $bp->pages->{$this->id}->slug : BP_DASHBOARD_SLUG,
			'has_directory'         => true, // Set to false if not required
			'notification_callback' => 'bp_dashboard_format_notifications',
			'search_string'         => __( 'Search dashboard...', 'buddypress' ),
	 ...
	 */
	function setup_nav() {
		// Add 'Dashboard' to the main navigation
		$main_nav = array(
			'name' 		      => __( 'Dashboard', 'bp-dashboard' ),
			'slug' 		      => bp_get_dashboard_slug(),
			'position' 	      => 80,
			'screen_function'     => 'bp_dashboard_home',
			'default_subnav_slug' => 'dashboard-home'
		);

		$dashboard_link = trailingslashit( bp_loggedin_user_domain() . bp_get_dashboard_slug() );

		// Add a few subnav items under the main Dashboard tab
		$sub_nav[] = array(
			'name'            =>  __( 'Dashboard Home', 'bp-dashboard' ),
			'slug'            => 'dashboard-home',
			'parent_url'      => $dashboard_link,
			'parent_slug'     => bp_get_dashboard_slug(),
			'screen_function' => 'bp_dashboard_home',
			'position'        => 10
		);
		...
		$sub_nav[] = array(
			'name'            =>  __( 'Screen Two', 'bp-dashboard' ),
			'slug'            => 'screen-two',
			'parent_url'      => $dashboard_link,
			'parent_slug'     => bp_get_dashboard_slug(),
			'screen_function' => 'bp_dashboard_screen_two',
			'position'        => 20
		);
		...
		// it like this. See bp_dashboard_screen_settings_menu() for more info
		bp_core_new_subnav_item( array(
			'name' 		  => __( 'Dashboard', 'bp-dashboard' ),
			'slug' 		  => 'dashboard-admin',
			'parent_slug'     => bp_get_settings_slug(),
			'parent_url' 	  => trailingslashit( bp_loggedin_user_domain() . bp_get_settings_slug() ),
			'screen_function' => 'bp_dashboard_screen_settings_menu',
			'position' 	  => 40,
	...
	function register_post_types() {
		...
		$labels = array(
			'name'	   => __( 'Pitches', 'bp-dashboard' ),
			'singular' => __( 'Pitch', 'bp-dashboard' )
		);

		// Set up the argument array for register_post_type()
		$args = array(
			'label'	   => __( 'Pitches', 'bp-dashboard' ),
			'labels'   => $labels,
			'public'   => false,
			'show_ui'  => true,
 ...
 * routine. Using print_r( $bp->dashboard ) or var_dump( $bp->dashboard ) at the end of this function
 ...
 *   	  global $bp;
 *	  var_dump( $bp->dashboard );
 *   }
 *   add_action( 'bp_init', 'bp_dashboard_var_dump' );
 * It goes without saying that you should not do this on a production site!
 *
 * @package BuddyPress_Dashboard_Component
 * @since 1.0
 */
function bp_dashboard_load_core_component() {
	global $bp;

	$bp->dashboard = new BP_Dashboard_Component;
}
add_action( 'bp_loaded', 'bp_dashboard_load_core_component' );

 ...
 * BP_Dashboard_Post object in get()) is one suggested implementation.
 */

class BP_Dashboard_Posts {
	var $id;
	var $sender_id;
	var $recipient_id;
	var $date;
	...
	function populate() {
		...
		if ( $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bp->dashboard->table_name} WHERE id = %d", $this->id ) ) ) {
	...
	function save() {
		 ...
		 * filter to 'bp-dashboard-filters.php'
		 *
		 *   add_filter( 'dashboard_data_fieldname1_before_save', 'wp_filter_kses' );
		 */

		$this->sender_id = apply_filters( 'bp_dashboard_data_sender_id_before_save', $this->sender_id, $this->id );
		$this->recipient_id  = apply_filters( 'bp_dashboard_data_recipient_id_before_save', $this->recipient_id, $this->id );
		$this->date	     = apply_filters( 'bp_dashboard_data_date_before_save', $this->date, $this->id );

		// Call a before save action here
		do_action( 'bp_dashboard_data_before_save', $this );
		...
		if ( $this->id ) {
			...
			$wp_update_post_args = array(
				...
				'post_title'	=> sprintf( __( '%1$s dashboard-posts %2$s', 'bp-dashboard' ), bp_core_get_user_displayname( $this->sender_id ), bp_core_get_user_displayname( $this->recipient_id ) )
			);
			...
			if ( $result ) {
				update_post_meta( $result, 'bp_dashboard_recipient_id', $this->recipient_id );
			}
		} else {
			// Set up the arguments for wp_insert_post()
		...
		do_action( 'bp_dashboard_data_after_save', $this );
	...
	/**
	 * Part of our bp_dashboard_has_posts() loop
	 *
	 * @package BuddyPress_Dashboard_Component
	 * @since 1.0
	 */

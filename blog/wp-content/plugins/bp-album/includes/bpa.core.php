...
function bp_album_setup_nav() {
	...
	$album_link_title = ($bp->displayed_user->id) ? bp_word_or_name( __( "My files", 'bp-album' ), __( "%s's pictures", 'bp-album' ) ,false,false) : __( "My files", 'bp-album' );
 ...
 * @since 0.1.8
 */
function bp_album_add_picture($owner_type,$owner_id,$title,$description,$priv_lvl,$date_uploaded,$pic_org_url,$pic_org_path,$pic_mid_url,$pic_mid_path,$pic_thumb_url,$pic_thumb_path,$embed_url){
...
function bp_album_record_activity($pic_data) {
	...
	$action = sprintf( __( '%s uploaded : %s', 'bp-album' ), bp_core_get_userlink($pic_data->owner_id), '<a href="'. $primary_link .'">'.$title.'</a>' );
	...
	// ===========================================================================================================
	 if(stripos($image_path,"jpg") || stripos($image_path,"jpeg") || stripos($image_path,"png") || stripos($image_path,"gif")) 
	{
		$content = '<p> <a href="'. $primary_link .'" class="picture-activity-thumb" title="'.$title.'"><img src="'. $image_path .'" /></a>'.$desc.'</p>';
	}
	else
	{
		$content = '<p> <a href="'. $primary_link .'" class="picture-activity-thumb" title="'.$title.'"><img src="'. get_template_directory_uri().'/images/video_dummy_image.jpg" /></a>'.$desc.'</p>';
	}	

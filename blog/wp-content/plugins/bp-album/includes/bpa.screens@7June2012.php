...
function bp_album_screen_edit_content() {
    ...
    <p>
	<label><?php _e('File activity and comments','bp-album') ?></label>
			...
			<?php _e('If File already has comments this will delete them','bp-album') ?>
    </p>
    <?php endif; ?>
...
function bp_album_screen_upload_content() {
	...
	<input type="text" name="embed_url" id="embed_url" value="" /></label>
    </p>
	...
	<input type="file" name="file" id="file" value=''/></label>
     ...
     <b>Allowed file types .png, .jpg, .gif, .mp3, .mp4, .flv, .mov</b>
	...
	<label><?php _e('Visibility','bp-album') ?></label>
			...
			<label>
			<input type="radio" name="privacy" value="<?php echo $k ?>" <?php
				if(!$checked){
    ...
    <input type="submit" name="submit" id="submit" value="<?php _e( 'Upload file', 'bp-album' ) ?>"/>
...
function bp_album_action_upload() {
	...
	if ( $bp->current_component == $bp->album->slug && $bp->album->upload_slug == $bp->current_action && isset( $_POST['submit'] )) {
		...
		$uploadErrors = array(
			...
			if ($_FILES['file']['name']!='' && $_REQUEST['embed_url']!=''){		
				$feedback_message[] = sprintf( __( 'Either use Embed code or upload files. you can not use both option at one time.', 'bp-album' ), $uploadErrors[7] );
				$error_flag = true;
			}
			else if ($_FILES['file']['name']=='' && $_REQUEST['embed_url']==''){		
				$feedback_message[] = sprintf( __( 'Either use Embed code or upload files.', 'bp-album' ), $uploadErrors[7] );
				$error_flag = true;
			}
			else if($_REQUEST['embed_url']!='')
			{	
				$vide_err = 0;											
				...
				/*if(trim($_REQUEST['video-description'])=='')
				{					
					$error_flag = true;
					$vide_err++;
				}*/	
				...
				if($vide_err>0)
				{
					$feedback_message[] =  __( 'Need Title and Description in case you are using Embed url', 'bp-album' );
				}												
			}
		/*	
		...
		*****************************commented by jaspreet 30May2012*******************************/
		...
		if(isset($_REQUEST['embed_url']) && $_REQUEST['embed_url']!='' && $error_flag == false)	
		{				
			global $current_user;						
			global $wpdb;
			
			$insert_embed_data = "insert into wp_bp_album(owner_type,owner_id,	date_uploaded,title,description,privacy,embed_url) values('user','".$current_user->ID."','".date('Y-m-d h:i:s')."','".$_REQUEST['video-title']."','".$_REQUEST['video-description']."','".$_REQUEST['privacy']."','".$_REQUEST['embed_url']."')";		

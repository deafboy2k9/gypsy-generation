	...
	if ( $bp->current_component == $bp->album->slug && $bp->album->upload_slug == $bp->current_action && isset( $_POST['submit'] )) {
		...
		$uploadErrors = array(
			...
			if ($_FILES['file']['name']!='' && $_REQUEST['embed_url']!='' && $_REQUEST['upload_url_field']!=''){		
				$feedback_message[] = sprintf( __( 'Please use only one field at a time.', 'bp-album' ), $uploadErrors[7] );
				$error_flag = true;
			}
			else if ($_FILES['file']['name']=='' && $_REQUEST['embed_url']=='' && $_REQUEST['upload_url_field']==''){		
				$feedback_message[] = sprintf( __( 'Please use at least one field.', 'bp-album' ), $uploadErrors[7] );
				$error_flag = true;
			}
		...
		*****************************commented by jaspreet 30May2012*******************************/
		...
		{	
			...
			$insert_embed_data1 = "insert into wp_bp_album(owner_type,owner_id,date_uploaded,title,pic_org_url,pic_org_path,pic_mid_url ,pic_mid_path ,pic_thumb_url ,pic_thumb_path,privacy) values('user','".$current_user->ID."','".gmdate( "Y-m-d H:i:s" )."','".$_REQUEST['upload_url_field']."','".$show_valid_image."','".$show_valid_image."','".$show_valid_image."','".$show_valid_image."','".$show_valid_image."','".$show_valid_image."','0')";		
		...
		{				
			...
			$insert_embed_data = "insert into wp_bp_album(owner_type,owner_id,	date_uploaded,title,privacy,embed_url) values('user','".$current_user->ID."','".gmdate( "Y-m-d H:i:s" )."','".$_REQUEST['video-title']."','0','".$_REQUEST['embed_url']."')";		
